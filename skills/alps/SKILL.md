...
**When improving an existing ALPS profile, ALWAYS follow this workflow:**

1. **Read handover.json if exists** - Check previous session's progress and advice (per ADR 0006)
2. **Identify gaps** - Explore website/documentation to find missing features
3. **Make improvements** - Add missing descriptors, enhance documentation, fix issues
4. **MANDATORY: Validate ALPS** - Run `asd --validate <file>` to ensure no errors
5. **MANDATORY: Generate HTML** - Run `asd <file> -o <file>.html` for visual review
6. **Update handover.json** - Record what was done and advice for next AI
7. **MANDATORY: Validate handover.json** - Verify JSON syntax after updating:
   ```bash
   node -e "JSON.parse(require('fs').readFileSync('handover.json', 'utf8')); console.log('✓ Valid JSON')"
   ```
8. **Report completion** - Provide before/after statistics and coverage estimation

**CRITICAL**: Steps 4-7 are MANDATORY for every improvement session. Never skip validation of both ALPS profile and handover.json.
...
- A descriptor MUST have exactly one of `id` or `href` (not both)
- `id` values must be unique within the document
- `href` must reference an existing `id` with `#` prefix (e.g., `{"href": "#userId"}`)
...
- Descriptors without nested `descriptor` should be on a single line
- Descriptors with nested `descriptor` may use multiple lines with indentation
...
Use `asd --validate <file>` to validate ALPS profiles. Output conforms to:
